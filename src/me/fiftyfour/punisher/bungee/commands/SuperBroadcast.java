...
public class SuperBroadcast extends Command {
    ...
    public void execute(CommandSender commandSender, String[] strings) {
        ...
        for (ProxiedPlayer all : ProxyServer.getInstance().getPlayers()) {
            ProxyServer.getInstance().createTitle().title().subTitle(new TextComponent(ChatColor.LIGHT_PURPLE + player.getName() + " has made an Announcement in chat!")).fadeIn(10).stay(100).fadeOut(10).send(all);
        }
    }
}